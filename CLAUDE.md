...
- O **Modo Timeline** (`timelineMode`, botão 🎬) é um **modo de gravação**: enquanto ativo, um relógio de rAF (`_startRecordClock`/`_stopRecordClock`) faz a agulha correr em tempo real. Esse relógio **só** chama `updatePlayhead`/`updateTimeDisplay` (via `_seekNeedle`) — nunca re-renderiza o canvas —, por isso convive sem conflito com o desenho ao vivo. O posicionamento dos traços é **sequencial** (não usa a posição livre da agulha): o início de cada traço vem de `_nextRecordStart()` = `max(_lastContentEnd(), _recordFloor)` — **derivado do conteúdo real**, não de um contador que só cresce, para que undo/redo, deleção de item e "limpar" reposicionem o cursor sozinhos (senão, apagar o último traço deixaria um vão antes do próximo). `_recordFloor` é o piso do início, definido ao entrar no modo, e guarda **só o offset do playhead** — nunca o fim do conteúdo (senão limpar/deletar o conteúdo dentro do modo deixaria um piso obsoleto e reabriria o vão); o fim do conteúdo entra dinamicamente via `_lastContentEnd()` dentro de `_nextRecordStart()`. Não sobrescrever conteúdo existente já é garantido por esse `max(_lastContentEnd(), _recordFloor)`. `stopDrawing` grava em `startTime = this._nextRecordStart()` com `duration` = tempo real de desenho (sem o ×0.5 do modo normal) e guarda `_recordCursor` = fim do traço (base da agulha durante a pausa); `startDrawing` recalcula e "prende" a agulha de volta a esse início ao começar o próximo traço. `totalAnimationTime` é estendido automaticamente se o conteúdo passar do fim. Ao mexer no fluxo de desenho, preserve o ramo `if (this.timelineMode)` nesses três pontos; ao adicionar novos jeitos de sair do modo, chame `_stopRecordClock()` para não deixar o rAF rodando.
