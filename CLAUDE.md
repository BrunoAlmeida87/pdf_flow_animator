...
- O **Modo Timeline** (`timelineMode`, botão 🎬) é um **modo de gravação**: enquanto ativo, um relógio de rAF (`_startRecordClock`/`_stopRecordClock`) faz a agulha correr em tempo real **enquanto um traço está sendo desenhado** (`isDrawing`). Fora do traço (ocioso, pausa) a agulha fica **parada** no ponto onde o próximo traço vai começar — o loop se auto-encerra quando `isDrawing` vira false (guard `!this.isDrawing` no `tick`), então ela **não** avança sozinha durante a pausa. Esse relógio **só** chama `updatePlayhead`/`updateTimeDisplay` (via `_seekNeedle`) — nunca re-renderiza o canvas —, por isso convive sem conflito com o desenho ao vivo. O posicionamento dos traços é **sequencial** (não usa a posição livre da agulha): o início de cada traço vem de `_nextRecordStart()` = `max(_lastContentEnd(), _recordFloor)` — **derivado do conteúdo real**, não de um contador que só cresce, para que undo/redo, deleção de item e "limpar" reposicionem o cursor sozinhos (senão, apagar o último traço deixaria um vão antes do próximo). `_recordFloor` é o piso do início, definido ao entrar no modo, e guarda **só o offset do playhead** — nunca o fim do conteúdo (senão limpar/deletar o conteúdo dentro do modo deixaria um piso obsoleto e reabriria o vão); o fim do conteúdo entra dinamicamente via `_lastContentEnd()` dentro de `_nextRecordStart()`. Não sobrescrever conteúdo existente já é garantido por esse `max(_lastContentEnd(), _recordFloor)`. `startDrawing` recalcula `_recordCursor = _nextRecordStart()`, prende a agulha ali e inicia o relógio (que avança porque `isDrawing` já é true); `stopDrawing` grava em `startTime = this._nextRecordStart()` com `duration` = tempo real de desenho (sem o ×0.5 do modo normal), guarda `_recordCursor` = fim do traço, **para o relógio** (`_stopRecordClock`) e prende a agulha nesse fim. `totalAnimationTime` é estendido automaticamente se o conteúdo passar do fim. Como a agulha ociosa é a **dica visual** de onde o próximo traço começa, edições que mudam o conteúdo (undo/redo, deleção/arraste de item, "limpar") reancoram a agulha via `_syncRecordNeedle()` — chamado no fim de `rebuildDrawingCanvas()` (chokepoint de toda mutação), com guard `timelineMode && !isDrawing && !isPlaying`. Ao mexer no fluxo de desenho, preserve o ramo `if (this.timelineMode)` nesses três pontos; ao adicionar novos jeitos de sair do modo, chame `_stopRecordClock()` para não deixar o rAF rodando.
